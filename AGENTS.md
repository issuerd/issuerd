...
- **Advanced OAuth** — PAR (RFC 9126); JWT client auth (`private_key_jwt`/`client_secret_jwt`); token exchange + impersonation (RFC 8693); offline tokens; per-realm signing algorithms (ES512 via `p521`); DPoP (RFC 9449; mTLS deferred); JAR + JARM + form_post; RAR (RFC 9396); dynamic client registration (RFC 7591/7592); CIBA (poll mode, `ext/ciba/auth` + `ext/ciba/approve`, DPoP-bound step-up tokens); pairwise subjects (OIDC Core §8). Agent-facing guides with recorded demos: `docs/agentic-iam-mcp.md`, `docs/ciba-step-up.md`.
