...
- **Subject-token audience is not checked by default** (divergence, opt-in strictness): Keycloak's standard token exchange requires the requesting client to be within the subject token's audience and denies the exchange otherwise. Issuerd accepts any valid access token of the realm regardless of its `aud`, preserving the behavior existing deployments rely on. The opt-in attribute `require_requester_in_subject_aud` switches to Keycloak's semantics: set as a realm attribute for a realm-wide default, or on the requesting client to override the realm setting in either direction (`"true"` enforces, `"false"` exempts). When enabled, the requester must appear in the subject token's `aud` (string or array form; `azp` is not considered) or the exchange — internal or impersonation — fails with `invalid_grant`.
