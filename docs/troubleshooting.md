...
| Login succeeds but the browser lands back on the login page (SSO loop) over plain HTTP | The SSO cookie is set as `issuerd_session_{realm-id}=...; HttpOnly; Secure; SameSite=Lax; Path=/` (`crates/issuerd-server/src/routes/login_api.rs`). Browsers drop `Secure` cookies on `http://` origins other than localhost, so the session never sticks. | Serve HTTPS (direct `[tls]` or a TLS-terminating proxy — see [deployment.md](deployment.md)), or use `localhost` for dev. |
